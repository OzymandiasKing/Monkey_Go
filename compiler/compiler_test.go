...
func TestLetStatementsScope(t *testing.T) {
	tests := []compilerTestCase{
		{
			input: `let num = 55;
					fn() { num; };
...
func TestMyBuiltins(t *testing.T) {
	tests := []compilerTestCase{
		{
			input: `
            len([1, 2, 3]);
...
func testInstructions(excepted []code.Instructions, actual code.Instructions) error {
	contacted := concatInstructions(excepted)
