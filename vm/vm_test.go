...
func TestMyBoolExpressions(t *testing.T) {
	tests := []vmTestCase{
		{"!(if (false) { 5; })", true},
	}
	runVmTests(t, tests)
...
func TestGlobalSetStatements(t *testing.T) {
	tests := []vmTestCase{
		{"let one = 1; one", 1},
...
func TestMyBuiltinFunctions(t *testing.T) {
	tests := []vmTestCase{
		{`rest([1, 2, 3])`, []int{2, 3}},
	}
	runVmTests(t, tests)
...
func runVmTests(t *testing.T, tests []vmTestCase) {
	...
	for _, tt := range tests {
		...
		// fmt.Printf("Bytecode for input `%s`:\n%s\n", tt.input, comp.Bytecode())
		vm := New(comp.Bytecode())
		err = vm.Run()
		if err != nil {
...
func testExpectedObject(t *testing.T, expected interface{}, actual object.Object) {
	...
	case map[object.HashKey]int64:
		...
		if len(hash.Pairs) != len(expected) {
			t.Errorf("hash has wrong num of Pairs. want=%d, got=%d", len(hash.Pairs), len(hash.Pairs))
			return
		}
